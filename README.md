...
The scripts contain automatic checks for illegal input combinations to flags or illegal flag combinations. However if you do discover a bug please submit a pull request directly on the github repository or email [Dr Kris Nikov](mailto:kris.nikov@bris.ac.uk) with details of the bug. The code is also heavily commented, so please use that as a further reference to the functional capabilities and options.
